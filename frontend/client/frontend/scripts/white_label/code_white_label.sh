...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]]; then
    ...
    sed $SED_INPLACE "s/const _localNotifierAppName = 'DSH Office'/const _localNotifierAppName = '$ESCAPED_COMPANY_NAME'/" "$CODE_FILE"
    if [ $? -ne 0 ]; then
    ...
    sed $SED_INPLACE "s/const _localNotifierAppName = 'DSH Office'/const _localNotifierAppName = '$CUSTOM_COMPANY_NAME'/" "$CODE_FILE"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to process $CODE_FILE with sed"
        exit 1
    fi
