...
class DocumentBloc extends Bloc<DocumentEvent, DocumentState> {
  ...
  DocEventPB? _pendingDocEvent;
  var _pendingExternalReconcile = false;
  ...
  Future<void> _onDocumentEvent(
    ...
    await event.when(
      initial: () async {
        ...
        emit(newState);
        if (newState.editorState != null &&
            databaseViewId == null &&
            rowId == null) {
          _museMarkdownSurface ??= MarkdownUiSurfaceBinding.open(
            documentId,
  ...
  void _throttleSyncDoc(DocEventPB docEvent) {
    ...
    _pendingDocEvent = docEvent;
    if (docEvent.isRemote) {
      _pendingExternalReconcile = true;
    }
    _syncThrottle.call(_flushThrottledDocEvent);
  }

  void _flushThrottledDocEvent() {
    final event = _pendingDocEvent;
    final external = _pendingExternalReconcile;
    _pendingDocEvent = null;
    _pendingExternalReconcile = false;
    if (event == null) {
      return;
    }
    if (external) {
      unawaited(_documentCollabAdapter.syncV3(docEvent: event));
      return;
    }
    unawaited(_onDocumentStateUpdate(event));
  }
  ...
  Future<void> _onSelectionUpdate() async {
    ...
    final museSurface = _museMarkdownSurface;
    if (editorState != null && museSurface != null) {
      unawaited(museSurface.publishSelection(editorState));
    }
