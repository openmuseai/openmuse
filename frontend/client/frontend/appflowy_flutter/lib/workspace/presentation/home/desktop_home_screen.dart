...
import 'package:appflowy/plugins/dsh_agent/dsh_agent_panel.dart';
import 'package:appflowy/plugins/dsh_agent/dsh_panel_resizer.dart';
...
class DesktopHomeScreen extends StatelessWidget {
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshots) {
        ...
        return AFFocusManager(
          child: ChangeNotifierProvider<DshAgentController>.value(
            value: getIt<DshAgentController>(),
            child: _DshAutostart(
              child: MultiBlocProvider(
                ...
                providers: [
                  ...
                  BlocProvider<HomeBloc>(
                    create: (_) => HomeBloc(workspaceLatest)
  ...
  Widget _layoutWidgets({
    ...
    final dsh = context.watch<DshAgentController>();
    return Row(
      children: [
        Expanded(
          child: Stack(
        ...
        if (dsh.open) ...[
          const DshPanelResizer(),
          SizedBox(
            width: dsh.width,
            child: const DshAgentPanel(),
          ),
        ],
      ],
    );
...
class _DshAutostart extends StatefulWidget {
  const _DshAutostart({required this.child});

  final Widget child;

  @override
  State<_DshAutostart> createState() => _DshAutostartState();
}

class _DshAutostartState extends State<_DshAutostart> {
  @override
  void initState() {
    super.initState();
    unawaited(_start());
  ...
  Future<void> _start() async {
    ...
    if (!mounted || !controller.open) return;
