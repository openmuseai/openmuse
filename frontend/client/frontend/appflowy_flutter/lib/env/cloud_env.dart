...
Future<AuthenticatorType> getAuthenticatorType() async {
  ...
  if (MuseCloudProfile.isConfigured) {
    // Preserve the user's explicit offline mode; do not restore a stale server.
    return value == '0'
...
class AppFlowyCloudSharedEnv {
  ...
  static Future<AppFlowyCloudSharedEnv> fromEnv() async {
    final remote = MuseCloudProfile.fromEnvironment();
    if (remote != null) {
      return AppFlowyCloudSharedEnv(
        authenticatorType: await getAuthenticatorType(),
        appflowyCloudConfig: _configurationFromProfile(remote),
      );
    }
    ...
    if (Env.enableCustomCloud) {
      ...
      final authType = AuthenticatorType.fromValue(Env.authenticatorType);
      late final AppFlowyCloudConfiguration appflowyCloudConfig;
      late final AuthenticatorType authenticatorType;

      if (authType == AuthenticatorType.appflowyCloudDevelop) {
        // OSS local dev: direct :8000 / :9999 ports (no nginx). Rust only accepts 0/2.
        final baseUrl =
            Env.afCloudUrl.isNotEmpty ? Env.afCloudUrl : 'http://localhost';
...
AppFlowyCloudConfiguration _configurationFromProfile(MuseCloudProfile profile) {
  return AppFlowyCloudConfiguration(
    base_url: profile.cloud.origin,
    gotrue_url: profile.gotrue.toString(),
    ws_base_url: profile.websocket.toString(),
    base_web_domain: profile.web.authority,
    enable_sync_trace: false,
  );
}
...
Future<AppFlowyCloudConfiguration> getAppFlowyCloudConfig(
  ...
  final remote = MuseCloudProfile.fromEnvironment();
  if (remote != null) return _configurationFromProfile(remote);
...
Future<String> getAppFlowyCloudUrl() async {
  final remote = MuseCloudProfile.fromEnvironment();
  if (remote != null) return remote.cloud.origin;
  final result =
...
Future<String> getAppFlowyShareDomain() async {
  final remote = MuseCloudProfile.fromEnvironment();
  if (remote != null) return remote.web.authority;
  final result =
