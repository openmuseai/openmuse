...
pub struct AppFlowyCore {
  ...
  /// Domain-neutral Agent Host registry; concrete Providers remain AppFlowy-owned adapters.
...
impl AppFlowyCore {
  ...
  async fn init(config: AppFlowyCoreConfig, runtime: Arc<AFPluginRuntime>) -> Self {
    ...
    let (muse_host_registry, muse_host_policy, muse_approval_secret) = muse_host::make_host_state(
      Arc::downgrade(&user_manager),
      Arc::downgrade(&folder_manager),
      Arc::downgrade(&server_provider),
    );
    let muse_host_events = Arc::new(
      muse_host_events::HostEventHub::new(muse_host_events::EventHubConfig::default())
        .expect("static Muse Host event configuration must be valid"),
    );
    muse_ui_context::install(muse_host_events.clone());
    let muse_host_providers = muse_view_reference::MuseHostProviders::register(
    ...
    let muse_host_runtime = muse_runtime::AppFlowyMuseRuntime::start(
      ...
      muse_host_policy,
      Arc::downgrade(&user_manager),
      muse_approval_secret,
      muse_host_events,
    )
    .map_err(|error| tracing::error!(?error, "failed to start AppFlowy Muse Host runtime"))
    .ok();
    let app_life_cycle = AppLifeCycleImpl {
      ...
      muse_host_registry: Arc::downgrade(&muse_host_registry),
    };
  ...
  pub async fn shutdown_muse_host(&self) {
    ...
    self.muse_host_providers.shutdown().await;
  }
}
