...
pub(crate) struct AppLifeCycleImpl {
  ...
  pub(crate) muse_host_registry: Weak<muse_host_registry::HostCapabilityRegistry>,
}
...
impl AppLifeCycle for AppLifeCycleImpl {
  ...
  async fn on_workspace_closed(&self, workspace_id: &Uuid) -> FlowyResult<()> {
    // Revoke before replacing the session or rebuilding workspace-scoped Managers.
    // A missed transport event is still safe because every invoke revalidates Host authority.
    if let (Some(registry), Ok(user_id)) = (
      self.muse_host_registry.upgrade(),
      self.user_manager()?.user_id(),
    ) {
      registry
        .invalidate_actor(&crate::muse_host::actor_ref(user_id))
        .await;
    }
