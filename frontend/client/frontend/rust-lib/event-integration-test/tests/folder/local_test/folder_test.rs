...
#[tokio::test]
async fn authorized_view_projection_applies_child_materialization_limit() {
  let mut test = FolderTest::new().await;
  let full = test
    .sdk
    .folder_manager
    .get_view_pb(&test.parent_view.id)
    .await
    .unwrap();
  assert_eq!(full.child_views.len(), 1);

  let bounded = test
    .sdk
    .folder_manager
    .get_view_pb_with_child_limit(&test.parent_view.id, 0)
    .await
    .unwrap();
  assert_eq!(bounded.id, full.id);
  assert!(bounded.child_views.is_empty());

  test.run_scripts(vec![DeleteParentView]).await;
  assert!(test
    .sdk
    .folder_manager
    .get_view_pb_with_child_limit(&test.parent_view.id, 1)
    .await
    .is_err());
}

#[tokio::test]
async fn compare_and_set_rename_rejects_stale_title_without_overwrite() {
  let test = FolderTest::new().await;
  test
    .sdk
    .folder_manager
    .rename_view_if_name(&test.parent_view.id, "first level view", "Renamed")
    .await
    .unwrap();
  let updated = test
    .sdk
  ...
  assert!(test
    .sdk
    .folder_manager
    .rename_view_if_name(&test.parent_view.id, "first level view", "Overwritten")
    .await
    .is_err());
  assert_eq!(
    test
