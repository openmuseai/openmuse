...
impl FolderManager {
  ...
  pub async fn get_view_pb(&self, view_id: &str) -> FlowyResult<ViewPB> {
    self.get_view_pb_with_child_limit(view_id, usize::MAX).await
  }

  /// Retrieves an authorized view while limiting how many immediate children
  /// are materialized into the returned protobuf. This is intended for bounded
  /// host projections; callers can request `limit + 1` to detect overflow.
  #[tracing::instrument(level = "debug", skip(self))]
  pub async fn get_view_pb_with_child_limit(
    &self,
    view_id: &str,
    child_limit: usize,
  ) -> FlowyResult<ViewPB> {
  ...
  /// Renames a visible View only when its title still matches `expected_name`.
  /// The comparison and Folder Collab mutation share one write lock so a Host
  /// command cannot overwrite a collaborator's intervening title change.
  #[tracing::instrument(level = "trace", skip(self), err)]
  pub async fn rename_view_if_name(
    &self,
    view_id: &str,
    expected_name: &str,
    new_name: &str,
  ) -> FlowyResult<()> {
    if expected_name.is_empty() || new_name.is_empty() {
      return Err(FlowyError::new(
    ...
    let value = match self.mutex_folder.load_full() {
      ...
      Some(lock) => {
        let mut folder = lock.write().await;
        let filtered = Self::get_view_ids_should_be_filtered(&folder);
        if filtered.iter().any(|id| id == view_id) {
          return Err(FlowyError::record_not_found());
        }
        let old_view = folder
          .get_view(view_id)
          .ok_or_else(FlowyError::record_not_found)?;
        if old_view.name != expected_name {
          return Err(FlowyError::new(ErrorCode::Conflict, "View title changed"));
        }
        if old_view.is_locked.unwrap_or(false) {
          return Err(FlowyError::view_is_locked());
        }
        let new_view = folder.update_view(view_id, |update| update.set_name(new_name).done());
        Some((old_view, new_view))
      },
    };

    if let Some((old_view, Some(new_view))) = value {
      self
        .get_handler(&old_view.layout)?
        .did_update_view(&old_view, &new_view)
        .await?;
    }
