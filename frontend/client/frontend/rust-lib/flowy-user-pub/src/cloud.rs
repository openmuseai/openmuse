...
/// Short-lived credential used by an untrusted presentation to attach to a
/// Muse Remote DSH session. The Cloud access token never crosses this seam.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct MuseDshDeviceToken {
  pub token: String,
  pub expires_at: i64,
...
pub trait UserCloudService: Send + Sync + 'static {
  /// Exchanges the current Cloud session for a short-lived, device-bound Muse
  /// credential. Non-Cloud backends fail closed.
