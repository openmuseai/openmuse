...
use flowy_user_pub::cloud::MuseDshDeviceToken;
...
where
  T: AFServer,
{
  async fn issue_muse_dsh_device_token(
    &self,
    device_id: &str,
    session_ref: &str,
  ) -> Result<MuseDshDeviceToken, FlowyError> {
    #[derive(serde::Deserialize)]
    struct MuseResponse<T> {
      data: Option<T>,
      code: i32,
    }
    ...
    let url = format!(
      "{}/api/muse/dsh/device-token",
      client.base_url().trim_end_matches('/')
    );
    let response = client
      .http_client_with_auth(reqwest::Method::POST, &url)
      .await?
      ...
      .send()
      .await
      .map_err(|_| FlowyError::internal().with_context("Muse device token request failed"))?
      .error_for_status()
      .map_err(|_| {
        FlowyError::unauthorized().with_context("Muse device token request was rejected")
      })?;
    let envelope: MuseResponse<MuseDshDeviceToken> = response
      .json()
      .await
      .map_err(|_| FlowyError::internal().with_context("Muse device token response was invalid"))?;
    if envelope.code != 0 {
      return Err(
        FlowyError::unauthorized().with_context("Muse device token exchange was rejected"),
      );
    }
    envelope.data.ok_or_else(|| {
      FlowyError::internal().with_context("Muse device token response is missing data")
    })
  }
