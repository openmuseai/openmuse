...
"{""data"":""A"",""field_type"":0}","{""data"":""1"",""field_type"":1}","{""data"":""1647251762"",""field_type"":2}","{""data"":"""",""field_type"":3}","{""data"":""edpw,cx0O"",""field_type"":4}","{""data"":""Yes"",""field_type"":5}","{""data"":""AppFlowy website - https://www.example.com"",""field_type"":6}","{""data"":""HLXi,CsGr,4WqN"",""field_type"":7}"
    ...
    for (index, row) in rows.iter().enumerate() {
      if let Some(cell) = row.cells.get(&field.id) {
        ...
        match &field_type {
          ...
          FieldType::URL => {
            if index == 0 {
              assert_eq!(s, "AppFlowy website - https://www.example.com");
            }
          },
          FieldType::Checklist
          | FieldType::LastEditedTime
