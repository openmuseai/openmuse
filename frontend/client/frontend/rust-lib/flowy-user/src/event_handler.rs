...
pub async fn issue_muse_dsh_device_token_handler(
  data: AFPluginData<IssueMuseDshDeviceTokenPB>,
  manager: AFPluginState<Weak<UserManager>>,
) -> DataResult<MuseDshDeviceTokenPB, FlowyError> {
  let manager = upgrade_manager(manager)?;
  let payload = data.into_inner();
  if payload.device_id.trim().is_empty() {
    return Err(FlowyError::invalid_data().with_context("device_id is required"));
  }
  let service = manager.cloud_service()?.get_user_service()?;
  let token = service
    .issue_muse_dsh_device_token(&payload.device_id, &payload.session_ref)
    .await?;
  data_result_ok(token.into())
}
