...
pub fn init(user_manager: Weak<UserManager>) -> AFPlugin {
  ...
  AFPlugin::new()
    ...
    .event(UserEvent::IssueMuseDshDeviceToken, issue_muse_dsh_device_token_handler)
}
...
pub enum UserEvent {
  ...
  /// Exchange the current Cloud session for a short-lived Remote DSH token.
  #[event(input = "IssueMuseDshDeviceTokenPB", output = "MuseDshDeviceTokenPB")]
  IssueMuseDshDeviceToken = 66,
}
