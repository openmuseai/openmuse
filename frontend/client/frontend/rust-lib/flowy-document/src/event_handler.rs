...
pub(crate) async fn publish_muse_ui_context_handler(
  data: AFPluginData<DocumentTextPB>,
) -> FlowyResult<()> {
  let value =
    serde_json::from_str(&data.into_inner().text).map_err(|_| FlowyError::invalid_data())?;
  crate::muse_context::publish(value)
    .map_err(|_| FlowyError::internal().with_context("Muse UI context publisher unavailable"))?;
  Ok(())
}
