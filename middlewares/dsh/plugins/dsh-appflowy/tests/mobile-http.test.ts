...
it("real HTTP/SSE carrier: capability, authenticated bind, intent, exclusion, scope and teardown", async () => {
  ...
  try {
    ...
    expect((await post(hello, { "X-Muse-Connection-Id": "connection.two" })).status).toBe(409);
    ...
    expect((await fetch(base, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source: "muse.appflowy-web", type: "workspace.bind", workspaceRef: "other" }) })).status).toBe(409);
    expect(await (await post({ source: "muse.appflowy-mobile", type: "peer.close" })).json()).toEqual({ ok: true });
    expect(removed).toContain("surface.appflowy.workspace.ws-1");
  } finally {
    await reader?.cancel();
    ...
    vi.restoreAllMocks();
    for (const key of ["MUSE_DOCUMENT_CLOUD_URL", "MUSE_PARENT_BRIDGE", "MUSE_MOBILE_BRIDGE_EXCLUSIVE_TEST", "MUSE_APPFLOWY_DSH_WORKSPACE_ROOT"]) {
