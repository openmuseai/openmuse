...
import { createPoolProxy, stripUpstreamAuth } from "../src/proxy.js";
...
describe("control HTTP + proxy", () => {
  ...
  it("forwards the public Host and splices WebSocket upgrades", async () => {
    const hash = "ab".repeat(16);
    const seen: { host?: string; url?: string; cookie?: string; upgrade?: boolean } = {};
    const backend = createServer((req, res) => {
    ...
    expect(seen.host).toBe("openmuseai.com");
